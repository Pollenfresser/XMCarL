...
const GActionEntry app_entries[] = {
	{ "End", menu_callback_end, NULL, NULL, NULL, {0,0,0} },
...
void menu_init (gpointer data)
{
	...
	gtk_application_set_accels_for_action (GTK_APPLICATION (a->app), "app.end", accels_end);
	gtk_application_set_accels_for_action (GTK_APPLICATION (a->app), "app.stream", accels_stream);
	gtk_application_set_accels_for_action (GTK_APPLICATION (a->app), "app.visual", accels_visual);
	gtk_application_set_accels_for_action (GTK_APPLICATION (a->app), "app.about", accels_about);

}

void menu_callback_end(GSimpleAction *action, GVariant *parameter, gpointer data)
{
	widgets *a = (widgets *) data;
	start_screen_init((gpointer) a);
}

void menu_callback_stream(GSimpleAction *action, GVariant *parameter, gpointer data)
{
	widgets *a = (widgets *) data;
	stream_screen_visible(NULL, (gpointer) a);
}

void menu_callback_visual(GSimpleAction *action, GVariant *parameter, gpointer data)
{
	widgets *a = (widgets *) data;
...
void menu_callback_about(GSimpleAction *action, GVariant *parameter, gpointer data)
{
	...
	gtk_show_about_dialog(GTK_WINDOW (a->window),
			      ...
			      "comments", "An application by some beginner-developers",
			      "authors", authors,
			      "logo", pixbuf,
			      NULL);

	g_signal_connect(GTK_DIALOG(about_dialog), "response", G_CALLBACK(destroy_dialog), (gpointer) a);
