...
gtk_widget_show_all(a->datavis.img);
  ...
  g_signal_connect(a->datavis.button, "clicked", G_CALLBACK(stream_screen_visible), (gpointer) a);


  gtk_box_pack_start (GTK_BOX(a->datavis.layout), a->datavis.button, FALSE, FALSE, 0);	
	

  gtk_box_pack_start (GTK_BOX(a->datavis.layout), a->datavis.label_name, FALSE, FALSE, 0);	
  gtk_box_pack_start (GTK_BOX(a->datavis.layout), a->datavis.img, FALSE, FALSE, 0);
