...
gtk_widget_show_all(a->start.img);
  ...
  a->start.button = gtk_button_new_with_label("Start Car");
  gtk_widget_set_size_request(a->start.button, 300, 100);
  g_signal_connect(a->start.button, "clicked", G_CALLBACK(stream_screen_visible), (gpointer) a);


  gtk_box_pack_start (GTK_BOX(a->start.layout), a->start.button, FALSE, FALSE, 0);	
	

  gtk_box_pack_start (GTK_BOX(a->start.layout), a->start.label_name, FALSE, FALSE, 0);	
